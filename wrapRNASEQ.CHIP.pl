...
#inputs
my $use = "\nUse: $0 options, gff path, fastq path, fastq name, gff name, read length, tag\n\n";
my ($options, $gffp, $fastqp, $fastq, $gff, $read,$tag) = @ARGV;
die $use unless (@ARGV == 7);
...
if ($options =~ /[CMF]/)
{
 ...
 $status{$n}=system "sh ".$scriptp."mapping.090511.sh";
 die "Problem stage $n" if $status{$n}; $n++;
